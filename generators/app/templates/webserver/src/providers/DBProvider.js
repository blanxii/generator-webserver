...
<% if(options.database == 'mysql' || options.database == 'postgresql' || options.database == 'sqlite'){ %>import Sequelize from 'sequelize';

container.register('SQLConnection', () => (
  new Sequelize(process.env.SQL_DATABASE, process.env.SQL_USER, process.env.SQL_PASSWORD, {
...
<% } %><% if(options.database == 'mongodb'){ %>import Promise from 'bluebird';
